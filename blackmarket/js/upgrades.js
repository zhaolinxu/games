...
var methPriceUpgrades = [
    ...
    new Upgrade("Biologist",		                750000000000000000,		    function() { drugMultiplier[1] *= 3 },    "冰毒价格 x3"),
    new Upgrade("Cessna Transport Aircraft",		2500000000000000000,	    function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
    new Upgrade("+5 Biologists",		            12500000000000000000,	    function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
    new Upgrade("Transport Mules",		            50000000000000000000,	    function() { drugMultiplier[1] *= 3 },    "冰毒价格 x3"),
    new Upgrade("Brand New Equipment",		        321000000000000000000,	    function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
	new Upgrade("Tubes, Lots of Tubes",		        750000000000000000000,	    function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
	new Upgrade("High-Tech Lab",	                1500000000000000000000,	    function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
	new Upgrade("Ivy League Chemists",	            5000000000000000000000,	    function() { drugMultiplier[1] *= 3 },    "冰毒价格 x3"),
	new Upgrade("Amezcua State-of-the-Art Lab",	    10000000000000000000000,	function() { drugMultiplier[1] *= 2 },    "冰毒价格 x2"),
...
var cocainePriceUpgrades = [
    new Upgrade("Jungle Lab",                           30000000000,			    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Small Chemical Lab",                   250000000000,			    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Packaging Machine",                    750000000000,     		    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Chemical Press",                       1500000000000,    		    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Trash Can o'Mixing",                   5000000000000,			    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Gasoline Container",		            25000000000000,			    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Cocaine Base Filter",		            750000000000000,		    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Cocaine Drying Machine",		        3500000000000000,		    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Hydraulic Press",		                75000000000000000,		    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Microwave Oven",		                500000000000000000,		    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Industrial Refrigerator",		        1000000000000000000,	    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Kidnap more Workers",		            25000000000000000000,	    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Field Expansion I",		            50000000000000000000,	    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Field Expansion II",		            125000000000000000000,	    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
    new Upgrade("Field Expansion III",		            654000000000000000000,	    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Field Expansion IV",		            1500000000000000000000,	    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
	new Upgrade("Kitchen Upgrade",		                2500000000000000000000,	    function() { drugMultiplier[2] *= 3 },    "可卡因价格 x3"),
    new Upgrade("Professional Underground Lab",		    5000000000000000000000,	    function() { drugMultiplier[2] *= 2 },    "可卡因价格 x2"),
...
Upgrade.init = function() {
	...
	for (var i = 0; i < shootRewardUpgrades.length; i++) {
		...
		$("#up-shoot-reward").append('<li id="up-shoot-reward-' + (i+1) + '" class="list-group-item cur-p"><b>' + s.name + ' </b>: ' + s.desc + ' - <i>花费 $' + fix(s.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < shootTimeUpgrades.length; i++) {
		...
		$("#up-shoot-time").append('<li id="up-shoot-time-' + (i+1) + '" class="list-group-item cur-p"><b>' + s.name + ' </b>: ' + s.desc + ' - <i>花费 $' + fix(s.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < ammoStockUpgrades.length; i++) {
		...
		$("#up-ammo-stock").append('<li id="up-ammo-stock-' + (i+1) + '" class="list-group-item cur-p"><b>' + a.name + ' </b>: ' + a.desc + ' - <i>花费 $' + fix(a.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < reloadTimeUpgrades.length; i++) {
		...
		$("#up-reload-time").append('<li id="up-reload-time-' + (i+1) + '" class="list-group-item cur-p"><b>' + r.name + ' </b>: ' + r.desc + ' - <i>花费 $' + fix(r.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < weedPriceUpgrades.length; i++) {
		...
		$("#up-weed-price").append('<li id="up-weed-price-' + (i+1) + '" class="list-group-item cur-p"><b>' + w.name + ' </b>: ' + w.desc + ' - <i>花费 $' + fix(w.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < methPriceUpgrades.length; i++) {
		...
		$("#up-meth-price").append('<li id="up-meth-price-' + (i+1) + '" class="list-group-item cur-p"><b>' + m.name + ' </b>: ' + m.desc + ' - <i>花费 $' + fix(m.price, "金钱") + '</i></li>');
	...
	for (var i = 0; i < cocainePriceUpgrades.length; i++) {
		...
		$("#up-cocaine-price").append('<li id="up-cocaine-price-' + (i+1) + '" class="list-group-item cur-p"><b>' + c.name + ' </b>: ' + c.desc + ' - <i>花费 $' + fix(c.price, "金钱") + '</i></li>');
